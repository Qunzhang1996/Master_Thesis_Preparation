...
import sys
path_to_add = r'C:\\Users\\86232\Desktop\\Master_Thesis\\robust-kalman'
...
B = np.array([[0.5 * dt**2], [dt]], np.float32)
...
kalman_linear = RobustKalman(F, B , H, x0_kalman, P0, Q0, R0, use_robust_estimation=False)
kalman_robust = RobustKalman(F, B , H, x0_kalman, P0, Q0, R0, use_robust_estimation=True)
...
for t in t_axis:
    ...
    q = np.random.normal(0, sigma_process, size=(1, 1))

    rare_event = 1 if np.random.uniform(0, 1.0) > 0.9 else 0
    r = np.random.normal(0.0, sigma_measure, size=(1, 1)) + np.random.choice([-10.0, 10.0]) * np.random.uniform(1.0, 1.5) * rare_event

    x = np.matmul(F, x) + np.matmul(G, q)
    z = np.matmul(H, x) + r

    kalman_linear.time_update(u)
    kalman_linear.measurement_update(z)
    kalman_robust.time_update(u)
